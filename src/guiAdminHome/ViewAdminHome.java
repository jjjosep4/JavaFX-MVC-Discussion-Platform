...
public class ViewAdminHome {
	...
	*/
	...
	// admin buttons to use to perform other roles.   fMany of these buttons are just stubs and an
	...
	private static final int theRole = 1;		// Admin: 1; RoleStudent: 2; RoleStaff: 3
	...
	*/
	...
	public static void displayAdminHome(Stage ps, User user) {
		...
		theDatabase.getUserAccountDetails(theUser.getUserName());		// Fetch this user's data
	...
	private ViewAdminHome() {
		...
		setupButtonUI(button_StaffRequests, "Dialog", 16, 250, Pos.CENTER, 320, 270);
		button_StaffRequests.setOnAction(e ->
		        guiAdminRequests.ViewAdminRequests.displayAdminRequests(theStage, theUser));
		theRootPane.getChildren().add(button_StaffRequests);

...
import guiUserLogin.PasswordModel;
import guiUserLogin.UserNameRecognizer;
...
public class ViewUserUpdate {
	...
	public static void displayUserUpdate(Stage ps, User user) {
    	...
    	if (theDatabase.getNumberOfUsers() == 1 
                && theUser.getAdminRole()) {
            // First user case logout after account information
            setupButtonUI(button_ProceedToUserHomePage, "Dialog", 18, 300, 
                    Pos.CENTER, width/2-150, 450);
            button_ProceedToUserHomePage.setText("Logout");
            button_ProceedToUserHomePage.setOnAction((event) -> {
                // Go back to login screen
                guiUserLogin.ViewUserLogin.displayUserLogin(theStage);
            });
        } else {
            // Normal user go to user home page
            setupButtonUI(button_ProceedToUserHomePage, "Dialog", 18, 300, 
                    Pos.CENTER, width/2-150, 450);
            button_ProceedToUserHomePage.setText("Return to User Home Page");
            button_ProceedToUserHomePage.setOnAction((event) -> 
	...
	private ViewUserUpdate() {
        ...
        button_UpdateUsername.setOnAction((event) -> {result = dialogUpdateUsername.showAndWait();
        	result.ifPresent(newUsername -> {
            	// Validate username
        	    String usernameErr = UserNameRecognizer.checkForValidUserName(newUsername);
        	    ...
        	    //if username is valid update the username
        		if (theDatabase.updateUsername(theUser.getUserName(), newUsername)) {
      ...
      //Validate the password
    	String passwordError = PasswordModel.evaluatePassword(newPassword);
    	...
    	//if password is valid update the password
		if (theDatabase.updatePassword(theUser.getUserName(), newPassword)) {
			theDatabase.getUserAccountDetails(newPassword);
        ...
        button_UpdateEmailAddress.setOnAction((event) -> {result = dialogUpdateEmailAddresss.showAndWait();
          ...
          //Validate the email address
        	String emailError = EmailAddressRecognizer.checkEmailAddress(newEmail);
        	...
        	//If email address is valid update to new email
        	if (theDatabase.updateEmailAddress(theUser.getUserName(), newEmail)) {
        	theDatabase.getUserAccountDetails(theUser.getUserName());
